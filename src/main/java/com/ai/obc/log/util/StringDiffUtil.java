...
public class StringDiffUtil {
    ...
    public String diff_prettyHtml(List<Diff> diffs) {
        ...
        for (Diff aDiff : diffs) {
            ...
            switch (aDiff.operation) {
                case INSERT:
                    html.append("<ins style=\"background:#ffe6e6;\">").append(text)
                            .append("</ins>");
                    break;
                case DELETE:
                    html.append("<del style=\"background:#e6ffe6;\">").append(text)
                            .append("</del>");
                    break;
                case EQUAL:
    ...
    public  String diffPrettyHtmlWithNoneDelete(String text1, String text2) {
        LinkedList<Diff>  diffs = diff_main(text1, text2);
        LinkedList<Diff>  noneDeleteDiffs = new LinkedList<>();
        for (Diff diff : diffs) {
            if (diff.operation != Operation.DELETE) {
                noneDeleteDiffs.add(diff);
            }
        }

        return diff_prettyHtml(noneDeleteDiffs);
    }
}